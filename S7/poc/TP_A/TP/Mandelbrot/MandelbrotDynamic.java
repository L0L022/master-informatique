...
public class MandelbrotDynamic extends Thread {
    ...
    final static int max = 205000;
    // C'est le nombre maximum d'itérations pour déterminer la couleur d'un pixel

    static volatile int nextLine = 0;

    public static void main(String[] args) throws InterruptedException {

        MandelbrotDynamic[] trs = new MandelbrotDynamic[4];

        Thread saveImages = new Thread(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < 1000 && nextLine < taille; ++i) {
                    image.save("pic" + String.format("%03d", i) + ".png");
                    try {
                        sleep(100);
    ...
    @Override
    public void run() {
        for (int i = getNextLine(); 0 <= i; i = getNextLine()) {
    ...
    private static synchronized int getNextLine() {
        if (nextLine >= taille)
            return -1;
        return nextLine++;
    }
